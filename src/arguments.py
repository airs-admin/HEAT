...
def get_args():
    ...
    parser.add_argument('--max_life_num', type=int, default=800000,
        help='max number of lifetime to train')
    ...
    parser.add_argument("--save_checkpoint", type=bool, default=False,
        help="Boolean flag to indicate whether to save a checkpoint after each model update during RL training")
    parser.add_argument("--memory", type=bool, default=True,
        help="enale hidden parameter")
    parser.add_argument('--h_dim', default=128,
        help='memory h dimension')
    parser.add_argument("--life_time_steps", type=int, default=1000,
        help="maximum number of children allowed at each node (optional; facilitate model loading if max_children is different at training time)")
    parser.add_argument("--train_freq", default=8, type=int,
        help="the number of life times to triger training.")
