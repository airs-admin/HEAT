...
ET.SubElement(torso, "joint", armature="0", axis="0 0 1", damping="0", limited="false", name="rootz", pos="0 0 0", ref=f"{torso_half_length + thigh_length + leg_length + 0.1}", stiffness="0", type="slide")
ET.SubElement(torso, "joint", armature="0", axis="0 1 0", damping="0", limited="false", name="rooty", pos=f"0 0 {torso_half_length + thigh_length + leg_length + 0.1}", stiffness="0", type="hinge")
ET.SubElement(torso, "geom", friction="0.9", fromto=f"0 0 {torso_length + thigh_length + leg_length + 0.1} 0 0 {thigh_length + leg_length + 0.1}", name="torso_geom", size="0.05", type="capsule")

# thigh element
thigh = ET.SubElement(torso, "body", name="thigh", pos=f"0 0 {thigh_length + leg_length + 0.1}")
ET.SubElement(thigh, "joint", axis="0 -1 0", name="thigh_joint", pos=f"0 0 {thigh_length + leg_length + 0.1}", range="-150 0", type="hinge")
ET.SubElement(thigh, "geom", friction="0.9", fromto=f"0 0 {thigh_length + leg_length + 0.1} 0 0 {leg_length + 0.1}", name="thigh_geom", size="0.05", type="capsule")

# leg element
leg = ET.SubElement(thigh, "body", name="leg", pos=f"0 0 {leg_length/2 + 0.1}")
ET.SubElement(leg, "joint", axis="0 -1 0", name="leg_joint", pos=f"0 0 {leg_length + 0.1}", range="-150 0", type="hinge")
ET.SubElement(leg, "geom", friction="0.9", fromto=f"0 0 {leg_length + 0.1} 0 0 {0.1}", name="leg_geom", size="0.04", type="capsule")

# foot element
foot = ET.SubElement(leg, "body", name="foot", pos="foot_length/6 0 0.1")
...
# List joint & actuator.
joint_options = [("thigh_joint", 200), ("leg_joint", 200), ("foot_joint", 200)]
selected_joints = []
unselected_joints = []
...
with open(log_file, "a") as log:
    log.write(f"hopper_4_{index}, range_len: {range_len}, disable_prob: {disable_prob}, "
...
output_file = os.path.join(output_dir, f"hopper_4_{index}.xml")
