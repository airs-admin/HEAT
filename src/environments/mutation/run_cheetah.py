...
def run_cheetah_scripts(num_runs, disable_prob, range_len, output_dir, log_file):
    # Define the script directory
    script_dir = "."

    # Get all cheetah script files
    cheetah_scripts = [
        f for f in os.listdir(script_dir)
        if f.startswith("random_cheetah_") and f.endswith(".py")
    ]

    if not cheetah_scripts:
        print("No cheetah scripts found in the specified directory.")
        return

    # Sort scripts by name to ensure consistent order
    cheetah_scripts.sort()

    for run in range(num_runs):
        for script in cheetah_scripts:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run multiple random cheetah generation scripts.")
    ...
    run_cheetah_scripts(args.num_runs, args.disable_prob, args.range_len, args.output_dir, args.log_file)