...
# bfoot element
bfoot_pos_1 = bfoot_length * math.sin(-0.27)
bfoot_pos_2 = bfoot_length * math.cos(-0.27)
bshin_2_bfoot_1 = 2 * bshin_pos_1
bshin_2_bfoot_2 = 2 * bshin_pos_2
bfoot = ET.SubElement(bshin, "body", name="bfoot", pos=f"{bshin_2_bfoot_1} 0 {bshin_2_bfoot_2}")
ET.SubElement(bfoot, "joint", axis="0 1 0", damping="3", name="bfoot", pos="0 0 0", range="-.4 .785", stiffness="120", type="hinge")
ET.SubElement(bfoot, "geom", axisangle="0 1 0 -.27", name="bfoot", pos=f"{-bfoot_pos_1} 0 {-bfoot_pos_2}", size=f"0.046 {bfoot_length}", type="capsule")
...
# fshin element
fshin_pos_1 = fshin_length * math.sin(-0.6)
fshin_pos_2 = fshin_length * math.cos(-0.6)
fthigh_2_fshin_pos_1 = 2 * fthigh_pos_1
fthigh_2_fshin_pos_2 = 2 * fthigh_pos_2
fshin = ET.SubElement(fthigh, "body", name="fshin", pos=f"{-fthigh_2_fshin_pos_1} 0 {-fthigh_2_fshin_pos_2}")
ET.SubElement(fshin, "joint", axis="0 1 0", damping="3", name="fshin", pos="0 0 0", range="-1.2 .87", stiffness="120", type="hinge")
ET.SubElement(fshin, "geom", axisangle="0 1 0 -.6", name="fshin", pos=f"{-fshin_pos_1} 0 {-fshin_pos_2}", size=f"0.046 {fshin_length}", type="capsule")

# ffoot element
ffoot_pos_1 = ffoot_length * math.sin(-0.6)
ffoot_pos_2 = ffoot_length * math.cos(-0.6)
fshin_2_ffoot_pos_1 = 2 * fshin_pos_1
fshin_2_ffoot_pos_2 = 2 * fshin_pos_2
ffoot = ET.SubElement(fshin, "body", name="ffoot", pos=f"{-fshin_2_ffoot_pos_1} 0 {-fshin_2_ffoot_pos_2}")
ET.SubElement(ffoot, "joint", axis="0 1 0", damping="1.5", name="ffoot", pos="0 0 0", range="-.5 .5", stiffness="60", type="hinge")
ET.SubElement(ffoot, "geom", axisangle="0 1 0 -.6", name="ffoot", pos=f"{-ffoot_pos_1} 0 {-ffoot_pos_2}", size=f"0.046 {ffoot_length}", type="capsule")
...
# List joint & actuator.
joint_options = [("bthigh", 120), ("bshin", 90), ("bfoot", 60), ("fthigh", 120), ("fshin", 60), ("ffoot", 30)]
selected_joints = []
unselected_joints = []
...
with open(log_file, "a") as log:
    log.write(f"cheetah_7_full_{index}, range_len: {range_len}, disable_prob: {disable_prob}, "
...
output_file = os.path.join(output_dir, f"cheetah_7_full_{index}.xml")
