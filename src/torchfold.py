        ...
        for arg in arg_lists:
            ...
            if all(isinstance(arg_item, Fold.Node) for arg_item in arg):
                ...
                if self._cuda:
                    ...
                    var = torch.tensor(arg, dtype=torch.long, device='cpu')
                res.append(var)
            else:
                for arg_item in arg:
                    ...
                    elif isinstance(arg_item, (torch.Tensor, Variable)):
                        r.append(arg_item)
                    else:
                        raise ValueError(
...
class Unfold(object):
    ...
    def _arg(self, arg):
        if isinstance(arg, Unfold.Node):
            ...
            if self._cuda:
                return torch.tensor([arg], dtype=torch.long, device='cuda')
            else:
                return torch.tensor([arg], dtype=torch.long, device='cpu')
        else:
            return arg
